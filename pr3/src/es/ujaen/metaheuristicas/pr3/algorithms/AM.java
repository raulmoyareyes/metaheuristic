...
import es.ujaen.metaheuristicas.pr3.clustering.Cluster;
...
public class AM {
     ...
     */
    public static Float init(String fileName, Integer seed, Integer numberClusters, Integer population, Float subgroup) {
        ...
        int positionBetter = run(populationClusters, populationCentroids, populationDistances, 
                populationChromosomes, numberClusters, patterns, rand, subgroup);
    ...
    public static Integer run(List<List<Cluster>> populationClusters, List<List<Pattern>> populationCentroids,
            List<Float> populationDistances, List<Chromosome> populationChromosomes, Integer numberClusters, 
            List<Pattern> patterns, Random rand, float subgroup) {
        ...
        for (int i = 0; i < 10000; i++) {
            ...
            generations++;
            if (generations % 10 == 0) {
                for (int j = 0; j < populationChromosomes.size() * subgroup; j++) {
                    float cost = BL.run(populationClusters.get(j), populationCentroids.get(j), populationDistances.get(j));
                    populationDistances.set(j, cost);
                    populationChromosomes.set(j, Functions.setChromosome(populationClusters.get(j), patterns, rand));
                }
            }
        }
